 ...
 * EsetNextStateach position stores a single cell
 *
...
public class Field {
    ...
    public List<Cell> getLivingNeighbours(Location location) {
        // diseased neighbours are counted as living neighbours 
        assert location != null : "Null location passed to adjacentLocations";
     ...
     * @return A list of living neighbours
     */
    public List<Cell> getNotDiseasedNeighbours(Location location) {
        assert location != null : "Null location passed to adjacentLocations";
        List<Cell> notDiseasedNeighbours = new LinkedList<>();
        ...
        if (location != null) {
            ...
            for (Location loc : adjLocations) {
                Cell cell = field[loc.getRow()][loc.getCol()];
                if (!cell.getDiseased() && cell.isAlive()) {
                    notDiseasedNeighbours.add(cell);
                }
            }
            Collections.shuffle(notDiseasedNeighbours, rand);
        }
        return notDiseasedNeighbours;
        }
     ...
     * @return A list of living neighbours
     */
    public List<Cell> getDiseasedNeighbours(Location location) 
    {
        assert location != null : "Null location passed to adjacentLocations";
        List<Cell> diseasedNeighbours = new LinkedList<>();
        ...
        if (location != null) {
            ...
            for (Location loc : adjLocations) {
                Cell cell = field[loc.getRow()][loc.getCol()];
                if (cell.getDiseased() && cell.isAlive())
                diseasedNeighbours.add(cell);
            }
            Collections.shuffle(diseasedNeighbours, rand);
        }
        return diseasedNeighbours;
    }
